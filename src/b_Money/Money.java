...
public class Money implements Comparable {
	...
	public String toString() {
		...
		if (amount < 0) cents = Math.abs(cents);
		
		String res = String.valueOf(units);
		if (cents == 0) {
			
		} else if (cents % 10 == 0) {
			res += "." + String.valueOf(cents / 10);
		} else {
			String centsStr = (cents < 10) ? "0" + String.valueOf(cents) : String.valueOf(cents);
			res += centsStr;
		}
		
		return res + " " + currency.getName();
	}
	...
	public Money add(Money other) {
		int otherInThis = currency.valueInThisCurrency(other.getAmount(), other.getCurrency());
		return new Money(amount + otherInThis, currency);
	}
	...
	public int compareTo(Object other) {
		...
		Integer thisUniversalValue = currency.universalValue(amount);
		Integer otherUniversalValue = otherMoney.getCurrency().universalValue(otherMoney.getAmount());
